...
print(f"  11 grados x {len(letras)} secciones = {11*len(letras)} secciones creadas")
...
# ══════════ PASO 3: CREAR ~3500 ESTUDIANTES UNICOS ══════════
print("[3/9] Creando ~3500 estudiantes unicos...")
# 3333 base + extras
TOTAL_UNIQUE = 3500
estudiantes_all = []
nombres_usados = set()
...
pool_2024 = estudiantes_all[:3333]

# Desercion 2024->2025: 100 alumnos salen
desertores_2025 = random.sample(pool_2024, 100)
...
nuevos_2025 = estudiantes_all[3333:3400]
pool_2025 = continuadores_2025 + nuevos_2025

# Desercion 2025->2026: 80 salen
desertores_2026 = random.sample(pool_2025, 80)
...
nuevos_2026 = estudiantes_all[3400:3500]
pool_2026 = continuadores_2026 + nuevos_2026
...
for anio, pool in pools.items():
    ...
    for gi, gk in enumerate(grado_keys):
        ...
        for j in range(n):
            if idx >= len(pool):
                break
            sec = secs[j % len(letras)]  # Round-robin A,B,C...
            est = pool[idx]
...
# ══════════ PASO 8.5: DOCENTES Y HORARIOS ══════════
print("[8.5/9] Creando Docentes y Horarios...")
from datetime import time
docentes_batch = []
for i in range(80):  # 80 docentes
    dni = gen_dni()
    nombre = random.choice(NM if i % 2 == 0 else NF)
    apellido = f"{random.choice(AP1)} {random.choice(AP2)}"
    espc = random.choice([c.nombre for c in cursos])
    docentes_batch.append(Docente(institucion=ie, dni=dni, nombres=f"{nombre} {apellido}", especialidad=espc, estado=True))

Docente.objects.bulk_create(docentes_batch)
docentes_all = list(Docente.objects.filter(institucion=ie))

dias = ["Lunes", "Martes", "Miercoles", "Jueves", "Viernes"]
horas = [(time(8,0), time(9,30)), (time(9,30), time(11,0)), (time(11,30), time(13,0))]
horarios_batch = []
all_secciones = list(Seccion.objects.filter(grado__nivel__institucion=ie))
for doc in docentes_all:
    # asignar de 3 a 5 secciones a cada docente para su curso
    curso_doc = next((c for c in cursos if c.nombre == doc.especialidad), cursos[0])
    secciones_doc = random.sample(all_secciones, k=random.randint(3, 5))
    for sec in secciones_doc:
        dia = random.choice(dias)
        h_i, h_f = random.choice(horas)
        horarios_batch.append(Horario(docente=doc, curso=curso_doc, seccion=sec, dia=dia, hora_inicio=h_i, hora_fin=h_f))

Horario.objects.bulk_create(horarios_batch)
print(f"  {len(docentes_all)} docentes y {len(horarios_batch)} horarios creados")
