...
\section{Mecanismos de Consistencia TPS y Propiedades ACID}
    ...
    Al interactuar con cualquier selector de esta cabecera temporal, el motor de procesamiento (SQL u OLAP VertiPaq) propaga automáticamente los filtros hacia todas las tarjetas KPI, gráficos de dispersión, tablas de aforo y fórmulas DAX, garantizando que el usuario examine el estado exacto de la escuela en la ventana de tiempo elegida.
    ...
    \section{Dashboard Operativo 2: Control Diario de Asistencia y Justificaciones (Auxiliaría)}

    Diseñado para los auxiliares de educación y la Secretaría del colegio:
    \begin{itemize}
        \item \textbf{Visualización:} Resumen de asistencia diaria desglosada por estado (Presente, Tardanza en puerta, Tardanza en aula, Inasistencia injustificada, Inasistencia justificada).
        \item \textbf{Monitoreo en Tiempo Real:} Marcación mediante escaneo de DNI/fotocheck o ingreso manual en aula, calculando el \% de asistencia diaria del colegio.
        \item \textbf{Buscador Rápido:} Consulta instantánea por estudiante para justificar faltas médicas adjuntando comprobantes digitales.
    \end{itemize}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.92\textwidth]{1.png}
        \caption{Dashboard Web Nativo 2: Control Diario de Asistencia y Justificaciones en Tiempo Real (Módulo Auxiliaría/Secretaría)}
        \label{fig:dashboard_web_caja}
    \end{figure}

    \begin{cajaConcepto}[Toma de Decisiones Operativas -- Dashboard Web 2 (Auxiliaría / Secretaría)]
    \textbf{Decisiones Inmediatas Habilitadas:}
    \begin{enumerate}
        \item \textbf{Cierre de Registro Diario de Puerta:} Valida la nómina de alumnos ingresados antes del inicio de la primera hora lectiva para enviar alertas preventivas a los apoderados.
        \item \textbf{Aprobación de Justificaciones Médicas:} Valida certificados presentados por los apoderados y desbloquea automáticamente los permisos de reprogramación de exámenes en el TPS Pedagógico.
        \item \textbf{Notificación Inmediata por Ausentismo:} Emite avisos automáticos al apoderado cuando el estudiante no registra ingreso en la puerta pasados 15 minutos del horario límite.
    \end{enumerate}
    \end{cajaConcepto}

    \section{Dashboard Operativo 3: Ficha Consolidada del Estudiante (Académico vs. Convivencia)}

    Sirve como el punto de contacto entre Dirección Académica y Secretaría/Tutoría al atender a un apoderado:
    \begin{itemize}
        \item \textbf{Vista Unificada:} Al ingresar el código del estudiante, la pantalla muestra en la mitad izquierda sus datos académicos (notas por competencia, promedio ponderado, evaluaciones pendientes) y en la mitad derecha su historial disciplinario (asistencia, tardanzas, méritos/deméritos).
        \item \textbf{Acción Directa:} Botón para emitir la Libreta Bimestral de Notas o generar una citación de tutoría presencial.
    \end{itemize}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.92\textwidth]{image.png}
        \caption{Dashboard Web Nativo 3: Ficha Consolidada del Estudiante (Interacción Dirección Académica -- Secretaría/Convivencia)}
        \label{fig:dashboard_web_ficha}
    \end{figure}

    \begin{cajaConcepto}[Toma de Decisiones Operativas -- Dashboard Web 3 (Ventanilla Única de Atención)]
    \textbf{Decisiones Inmediatas Habilitadas:}
    \begin{enumerate}
        \item \textbf{Emisión de Libretas y Constancias:} Emite automáticamente la libreta bimestral oficial y constancias de conducta consolidadas.
        \item \textbf{Generación de Compromisos de Tutoría:} Permite suscribir acuerdos pedagógicos y disciplinarios con el apoderado ante bajas calificaciones o faltas reiteradas.
    \end{enumerate}
    \end{cajaConcepto}
    ...
    Muestra un gráfico de barras apiladas al 100\% con el rendimiento por grado (Logro Destacado AD vs. Logrado A vs. En Proceso B vs. En Inicio C). Incluye segmentadores por nivel educativo.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.95\textwidth]{3.png}
        \caption{Power BI Dashboard 1: Radar de Rendimiento Académico y Desaprobación por Grado}
        \label{fig:powerbi_radar_morosidad}
    \end{figure}

    \begin{cajaConcepto}[Toma de Decisiones Estratégicas -- Power BI Dashboard 1 (Dirección Académica)]
    \textbf{Decisiones Estratégicas Habilitadas:}
    \begin{enumerate}
        \item \textbf{Reestructuración del Plan Pedagógico ($\text{PDC}_c > 20\%$):} Identifica asignaturas con alta tasa de desaprobación para programar capacitaciones docentes y talleres de reforzamiento.
        \item \textbf{Ajuste Curricular Bimestral:} Permite reasignar contenidos temáticos de mayor complejidad según los resultados de logro de competencias.
    \end{enumerate}
    \end{cajaConcepto}

    \subsection{Dashboard Power BI 2: Matriz de Eficiencia Operativa y Aforo por Sección}
    Gráfico de dispersión que evalúa el Aforo Ocupado (eje X) frente al Promedio de Rendimiento (eje Y). Permite a la Dirección identificar secciones con alto rendimiento o sobreaforo.

    [id_colegio] = USERPRINCIPALNAME()
    \end{lstlisting}

    De esta manera, cuando el Director de una institución abre el reporte de Power BI, el motor analítico filtra automáticamente los datos pertenecientes a su colegio.
    ...
    \begin{enumerate}
        \item \textbf{Integración de Sistemas de Información Desacoplados:} La arquitectura de SICOLEGIO 360 demuestra que la gestión educativa no requiere forzar un ERP centralizado monolítico. La interacción de dos \textbf{Sistemas Transaccionales (TPS)} independientes —el \textbf{TPS Pedagógico} (Dirección Académica) y el \textbf{TPS de Control Escolar} (Secretaría y Convivencia)— permite mantener la autonomía operativa de cada área.
        \item \textbf{Robustez Transaccional mediante TPS y ACID:} La implementación de un motor TPS sustentado en los principios ACID mediante transacciones SQL y disparadores asegura que eventos críticos (como la aprobación de justificaciones médicas) actualicen automáticamente los derechos de evaluación en el registro auxiliar del docente, eliminando la duplicidad de digitación y los ceros indebidos.
        \item \textbf{Articulación de la Capa de Métricas e Indicadores:} El establecimiento formal de KPIs como la Tasa de Asistencia Ponderada ($\text{TAP}$), el Porcentaje de Desaprobación por Curso ($\text{PDC}$) y el Índice de Incidencias de Conducta ($\text{ICG}$) permite transformar las marcaciones diarias en conocimiento valioso para tutores y coordinadores.
        \item \textbf{Diferenciación entre Reporting Web Nativo (MIS) y Power BI (DSS):} El documento sustenta la importancia de separar los dashboards web de ventanilla única (latencia $<1$ segundo para auxiliares y secretarios) de los paneles multidimensionales en Power BI. El uso de esquemas en estrella y expresiones DAX permite a la alta dirección correlacionar el ausentismo con el rendimiento académico sin degradar la base operativa.
        \item \textbf{Optimización de la Experiencia Escolar y Atención a Familias:} La integración transaccional permite atender al apoderado mediante una Ventanilla Única, emitiendo libretas, justificaciones y citaciones en tiempo real de forma transparente.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item \textbf{Implantación Socio-Tecnológica y Capacitación Continuada:} Se recomienda acompañar el despliegue de SICOLEGIO 360 con programas de gestión del cambio para que docentes y auxiliares efectúen el registro de notas y asistencia de forma directa en el aula.
        \item \textbf{Mantenimiento y Respaldo de Bases de Datos:} Mantener políticas estrictas de mantenimiento de la base de datos (SQLite/PostgreSQL), optimizando periódicamente los índices relacionales para soportar picos transaccionales durante cierres de bimestre.
        \item \textbf{Gobernanza de Datos y Seguridad RLS en Power BI:} Establecer auditorías periódicas a las reglas de Seguridad a Nivel de Fila (\textit{Row-Level Security}) en Power BI para garantizar el aislamiento estricto entre instituciones educativas.
        \item \textbf{Monitoreo Normativo de Protección de Datos:} Cumplir con la Ley N° 29733 (Ley de Protección de Datos Personales), asegurando el tratamiento confidencial del legajo del estudiante y las calificaciones.
    \end{enumerate}
    ...
    -- Tabla de Calificaciones y Evaluaciones (Académico - TPS 1)
    CREATE TABLE notas (
        id_nota INT AUTO_INCREMENT PRIMARY KEY,
        id_estudiante INT NOT NULL,
        id_curso INT NOT NULL,
        calificacion_numerica DECIMAL(4,2) NOT NULL,
        permite_reprogramacion BOOLEAN DEFAULT FALSE,
        FOREIGN KEY (id_estudiante) REFERENCES estudiantes(id_estudiante)
    );
    \end{lstlisting}

    \section*{Anexo B: Trigger Transaccional para Habilitación Atómica de Reprogramación de Evaluación}

    \begin{lstlisting}[language=SQL, caption={Trigger SQL para la integracion atomica entre Justificacion de Faltas y Evaluaciones}]
    DELIMITER //
    CREATE TRIGGER trg_aprobar_justificacion_evaluacion
    AFTER INSERT ON justificaciones
    FOR EACH ROW
    BEGIN
        -- Si la justificación es aprobada por Secretaría
        IF NEW.estado = 'APROBADO' THEN
            -- Habilitar la reprogramación de evaluaciones en el TPS Pedagógico del docente
            UPDATE notas n
            INNER JOIN asistencias a ON n.id_estudiante = a.id_estudiante
            SET n.permite_reprogramacion = TRUE
